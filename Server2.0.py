...
try:
    ...
    duco_email = "duco@gmail"
    duco_password = "Secure123"
    NodeS_Overide ="Secure123"
    wrapper_private_key = "Secure123"
...
blocks = query("SELECT blocks FROM Server", fetch='one')
lastBlockHash = query("SELECT lastBlockHash FROM Server", fetch='one')
...
def getRegisteredUsers():
    while True:
        try:
            registeredUsers = query("SELECT COUNT(username) FROM Users", fetch='one')
            break
        except:
            pass
...
def getMinedDuco():
    while True:
        try:
            allMinedDuco = query("SELECT SUM(balance) FROM Users", fetch='one')
            break
        except:
            pass
    return allMinedDuco
def getLeaders():
    while True:
        try:
            leadersdata = []
            for row in query("SELECT * FROM Users ORDER BY balance DESC", fetch='all'):
...
def getTransactions():
    while True:
        try:
            ...
            for row in query("SELECT * FROM Transactions", fetch='all'):
...
def getBlocks():
    while True:
        try:
            ...
            for row in query("SELECT * FROM Blocks", fetch='all'):
            ...
            if str(data[0]) == "SEND" and str(username) != "":
                ...
                if str(amount) == "" or str(recipient) == "" or float(balance) <= float(amount) or float(amount) <= 0:
                    try:
                        ...
                        break
                try:
                    recipientbal = float(query(f"SELECT * FROM Users WHERE username = {recipient}", fetch='one')[3]) # Get receipents' balance
